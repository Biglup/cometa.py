...
class StakeVoteDelegationCert:
    """
    Represents a combined stake and vote delegation certificate.

    This certificate allows delegating both stake (to a pool) and voting power
    (to a DRep) in a single certificate.
    """

    def __init__(self, ptr) -> None:
        if ptr == ffi.NULL:
            raise CardanoError("StakeVoteDelegationCert: invalid handle")
        self._ptr = ptr

    def __del__(self) -> None:
        if getattr(self, "_ptr", ffi.NULL) not in (None, ffi.NULL):
            ptr_ptr = ffi.new("cardano_stake_vote_delegation_cert_t**", self._ptr)
            lib.cardano_stake_vote_delegation_cert_unref(ptr_ptr)
            self._ptr = ffi.NULL

    def __enter__(self) -> StakeVoteDelegationCert:
        return self
    ...
    @classmethod
    def new(
        cls, credential: Credential, pool_key_hash: Blake2bHash, drep: DRep
    ) -> StakeVoteDelegationCert:
        """
        Creates a new stake and vote delegation certificate.

        Args:
            credential: The stake credential delegating stake and voting power.
        ...
        err = lib.cardano_stake_vote_delegation_cert_new(
            credential._ptr, pool_key_hash._ptr, drep._ptr, out
        )
        if err != 0:
            raise CardanoError(
                f"Failed to create StakeVoteDelegationCert (error code: {err})"
            )
        return cls(out[0])

    @classmethod
    def from_cbor(cls, reader: CborReader) -> StakeVoteDelegationCert:
        """
        ...
        """
        out = ffi.new("cardano_stake_vote_delegation_cert_t**")
        err = lib.cardano_stake_vote_delegation_cert_from_cbor(reader._ptr, out)
        if err != 0:
            raise CardanoError(
                f"Failed to deserialize StakeVoteDelegationCert from CBOR (error code: {err})"
            )
        return cls(out[0])
    ...
    def to_cbor(self, writer: CborWriter) -> None:
        ...
        """
        err = lib.cardano_stake_vote_delegation_cert_to_cbor(self._ptr, writer._ptr)
        if err != 0:
            raise CardanoError(
                f"Failed to serialize StakeVoteDelegationCert to CBOR (error code: {err})"
            )
    ...
    def credential(self) -> Credential:
        ...
        """
        ptr = lib.cardano_stake_vote_delegation_cert_get_credential(self._ptr)
        if ptr == ffi.NULL:
    ...
    def credential(self, value: Credential) -> None:
        ...
        """
        err = lib.cardano_stake_vote_delegation_cert_set_credential(
    ...
    def pool_key_hash(self) -> Blake2bHash:
        ...
        """
        ptr = lib.cardano_stake_vote_delegation_cert_get_pool_key_hash(self._ptr)
        if ptr == ffi.NULL:
    ...
    def pool_key_hash(self, value: Blake2bHash) -> None:
        ...
        Raises:
            CardanoError: If setting fails.
        """
        err = lib.cardano_stake_vote_delegation_cert_set_pool_key_hash(
            self._ptr, value._ptr
        )
        if err != 0:
    ...
    def drep(self) -> DRep:
        ...
        """
        ptr = lib.cardano_stake_vote_delegation_cert_get_drep(self._ptr)
        if ptr == ffi.NULL:
    ...
    def drep(self, value: DRep) -> None:
        ...
        Raises:
            CardanoError: If setting fails.
        """
        err = lib.cardano_stake_vote_delegation_cert_set_drep(self._ptr, value._ptr)
        if err != 0:
