...
class VoteRegistrationDelegationCert:
    """
    Represents a combined vote registration and delegation certificate.

    This certificate allows registering a stake credential and delegating
    voting power to a DRep in a single certificate.
    """

    def __init__(self, ptr) -> None:
        if ptr == ffi.NULL:
            raise CardanoError("VoteRegistrationDelegationCert: invalid handle")
        self._ptr = ptr

    def __del__(self) -> None:
        if getattr(self, "_ptr", ffi.NULL) not in (None, ffi.NULL):
            ptr_ptr = ffi.new("cardano_vote_registration_delegation_cert_t**", self._ptr)
            lib.cardano_vote_registration_delegation_cert_unref(ptr_ptr)
            self._ptr = ffi.NULL

    def __enter__(self) -> VoteRegistrationDelegationCert:
        return self
    ...
    def __repr__(self) -> str:
        return f"VoteRegistrationDelegationCert(deposit={self.deposit})"

    @classmethod
    def new(
        cls, credential: Credential, deposit: int, drep: DRep
    ) -> VoteRegistrationDelegationCert:
        """
        Creates a new vote registration and delegation certificate.

        Args:
            credential: The stake credential to register.
        ...
        err = lib.cardano_vote_registration_delegation_cert_new(
            credential._ptr, deposit, drep._ptr, out
        )
        if err != 0:
            raise CardanoError(
                f"Failed to create VoteRegistrationDelegationCert (error code: {err})"
            )
        return cls(out[0])

    @classmethod
    def from_cbor(cls, reader: CborReader) -> VoteRegistrationDelegationCert:
        """
        ...
        """
        out = ffi.new("cardano_vote_registration_delegation_cert_t**")
        err = lib.cardano_vote_registration_delegation_cert_from_cbor(reader._ptr, out)
        if err != 0:
            raise CardanoError(
                f"Failed to deserialize VoteRegistrationDelegationCert from CBOR (error code: {err})"
            )
        return cls(out[0])
    ...
    def to_cbor(self, writer: CborWriter) -> None:
        ...
        """
        err = lib.cardano_vote_registration_delegation_cert_to_cbor(
            self._ptr, writer._ptr
        )
        if err != 0:
            raise CardanoError(
                f"Failed to serialize VoteRegistrationDelegationCert to CBOR (error code: {err})"
            )
    ...
    def credential(self) -> Credential:
        ...
        """
        ptr = lib.cardano_vote_registration_delegation_cert_get_credential(self._ptr)
        if ptr == ffi.NULL:
    ...
    def credential(self, value: Credential) -> None:
        ...
        """
        err = lib.cardano_vote_registration_delegation_cert_set_credential(
    ...
    def deposit(self) -> int:
        ...
        Returns:
            The deposit amount.
        """
        return int(lib.cardano_vote_registration_delegation_cert_get_deposit(self._ptr))
    ...
    def deposit(self, value: int) -> None:
        ...
        Raises:
            CardanoError: If setting fails.
        """
        err = lib.cardano_vote_registration_delegation_cert_set_deposit(
            self._ptr, value
        )
        if err != 0:
    ...
    def drep(self) -> DRep:
        ...
        """
        ptr = lib.cardano_vote_registration_delegation_cert_get_drep(self._ptr)
        if ptr == ffi.NULL:
    ...
    def drep(self, value: DRep) -> None:
        ...
        Raises:
            CardanoError: If setting fails.
        """
        err = lib.cardano_vote_registration_delegation_cert_set_drep(
            self._ptr, value._ptr
        )
        if err != 0:
