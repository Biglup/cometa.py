...
class UpdateDRepCert:
    """
    Represents a DRep update certificate.

    This certificate is used when a Delegated Representative (DRep) wants to
    update their metadata anchor.
    """

    def __init__(self, ptr) -> None:
        if ptr == ffi.NULL:
            raise CardanoError("UpdateDRepCert: invalid handle")
        self._ptr = ptr

    def __del__(self) -> None:
        if getattr(self, "_ptr", ffi.NULL) not in (None, ffi.NULL):
            ptr_ptr = ffi.new("cardano_update_drep_cert_t**", self._ptr)
            lib.cardano_update_drep_cert_unref(ptr_ptr)
            self._ptr = ffi.NULL

    def __enter__(self) -> UpdateDRepCert:
        return self
    ...
    @classmethod
    def new(
        cls, credential: Credential, anchor: Optional[Anchor] = None
    ) -> UpdateDRepCert:
        """
        Creates a new DRep update certificate.

        Args:
            credential: The DRep credential to update.
            anchor: Optional new anchor with DRep metadata.
        ...
        err = lib.cardano_update_drep_cert_new(credential._ptr, anchor_ptr, out)
        if err != 0:
            raise CardanoError(f"Failed to create UpdateDRepCert (error code: {err})")
        return cls(out[0])

    @classmethod
    def from_cbor(cls, reader: CborReader) -> UpdateDRepCert:
        """
        ...
        """
        out = ffi.new("cardano_update_drep_cert_t**")
        err = lib.cardano_update_drep_cert_from_cbor(reader._ptr, out)
        if err != 0:
            raise CardanoError(
                f"Failed to deserialize UpdateDRepCert from CBOR (error code: {err})"
            )
        return cls(out[0])
    ...
    def to_cbor(self, writer: CborWriter) -> None:
        ...
        """
        err = lib.cardano_update_drep_cert_to_cbor(self._ptr, writer._ptr)
        if err != 0:
            raise CardanoError(
                f"Failed to serialize UpdateDRepCert to CBOR (error code: {err})"
            )
    ...
    def credential(self) -> Credential:
        ...
        """
        ptr = lib.cardano_update_drep_cert_get_credential(self._ptr)
        if ptr == ffi.NULL:
    ...
    def credential(self, value: Credential) -> None:
        ...
        """
        err = lib.cardano_update_drep_cert_set_credential(self._ptr, value._ptr)
        if err != 0:
    ...
    def anchor(self) -> Optional[Anchor]:
        ...
        """
        ptr = lib.cardano_update_drep_cert_get_anchor(self._ptr)
        if ptr == ffi.NULL:
            return None
    ...
    def anchor(self, value: Optional[Anchor]) -> None:
        ...
        err = lib.cardano_update_drep_cert_set_anchor(self._ptr, anchor_ptr)
        if err != 0:
