...
class VkeyWitnessSet:
    """
    Represents a set of verification key witnesses.

    This collection type is used in transaction witness sets to hold all
    VKey witnesses (public key + signature pairs) that authorize the transaction.
    """

    def __init__(self, ptr=None) -> None:
        if ptr is None:
            out = ffi.new("cardano_vkey_witness_set_t**")
            err = lib.cardano_vkey_witness_set_new(out)
            if err != 0:
                raise CardanoError(
                    f"Failed to create VkeyWitnessSet (error code: {err})"
                )
            self._ptr = out[0]
        else:
            if ptr == ffi.NULL:
                raise CardanoError("VkeyWitnessSet: invalid handle")
            self._ptr = ptr

    def __del__(self) -> None:
        if getattr(self, "_ptr", ffi.NULL) not in (None, ffi.NULL):
            ptr_ptr = ffi.new("cardano_vkey_witness_set_t**", self._ptr)
            lib.cardano_vkey_witness_set_unref(ptr_ptr)
            self._ptr = ffi.NULL

    def __enter__(self) -> VkeyWitnessSet:
        return self
    ...
    def __repr__(self) -> str:
        return f"VkeyWitnessSet(len={len(self)})"

    @classmethod
    def from_cbor(cls, reader: CborReader) -> VkeyWitnessSet:
        """
        ...
        """
        out = ffi.new("cardano_vkey_witness_set_t**")
        err = lib.cardano_vkey_witness_set_from_cbor(reader._ptr, out)
        if err != 0:
            raise CardanoError(
    ...
    def to_cbor(self, writer: CborWriter) -> None:
        ...
        """
        err = lib.cardano_vkey_witness_set_to_cbor(self._ptr, writer._ptr)
        if err != 0:
            raise CardanoError(
                f"Failed to serialize VkeyWitnessSet to CBOR (error code: {err})"
            )

    def add(self, witness: VkeyWitness) -> None:
        """
        Adds a VKey witness to the set.

        Args:
            witness: The VkeyWitness to add.

        Raises:
            CardanoError: If addition fails.
        """
        err = lib.cardano_vkey_witness_set_add(self._ptr, witness._ptr)
        if err != 0:
            raise CardanoError(f"Failed to add to VkeyWitnessSet (error code: {err})")

    def get(self, index: int) -> VkeyWitness:
        """
        ...
        err = lib.cardano_vkey_witness_set_get(self._ptr, index, out)
        if err != 0:
            raise CardanoError(f"Failed to get from VkeyWitnessSet (error code: {err})")
        return VkeyWitness(out[0])
    ...
    def use_tag(self) -> bool:
        ...
        """
        return bool(lib.cardano_vkey_witness_set_get_use_tag(self._ptr))
    ...
    def use_tag(self, value: bool) -> None:
        ...
        """
        err = lib.cardano_vkey_witness_set_set_use_tag(self._ptr, value)
        if err != 0:
            raise CardanoError(f"Failed to set use_tag (error code: {err})")

    def __len__(self) -> int:
        """Returns the number of witnesses in the set."""
        return int(lib.cardano_vkey_witness_set_get_length(self._ptr))

    def __iter__(self) -> Iterator[VkeyWitness]:
        """Iterates over all witnesses in the set."""
        for i in range(len(self)):
            yield self.get(i)

    def __getitem__(self, index: int) -> VkeyWitness:
    ...
    def to_cip116_json(self, writer: "JsonWriter") -> None:
        ...
        err = lib.cardano_vkey_witness_set_to_cip116_json(self._ptr, writer._ptr)
        if err != 0:
