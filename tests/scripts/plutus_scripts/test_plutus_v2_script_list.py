...
PLUTUS_V2_SCRIPT1_CBOR = "4e4d01000033222220051200120011"
PLUTUS_V2_SCRIPT2_CBOR = "4e4d01000133222220051200120011"
PLUTUS_V2_SCRIPT3_CBOR = "4e4d01000233222220051200120011"
PLUTUS_V2_SCRIPT4_CBOR = "4e4d01000333222220051200120011"
EMPTY_LIST_CBOR = "80"


def new_default_plutus_v2_script(cbor_hex: str) -> PlutusV2Script:
    """Creates a new PlutusV2Script from CBOR hex string."""
    reader = CborReader.from_hex(cbor_hex)
    return PlutusV2Script.from_cbor(reader)


class TestPlutusV2ScriptListNew:
    """Tests for PlutusV2ScriptList.__init__() constructor."""

    def test_can_create_empty_list(self):
        """Test that an empty PlutusV2ScriptList can be created."""
    ...
    def test_can_create_multiple_independent_lists(self):
        ...
        list1 = PlutusV2ScriptList()
        list2 = PlutusV2ScriptList()
        assert list1 is not None
        assert list2 is not None
        assert list1 is not list2


class TestPlutusV2ScriptListFromList:
    """Tests for PlutusV2ScriptList.from_list() factory method."""

    def test_can_create_from_empty_python_list(self):
        """Test that PlutusV2ScriptList can be created from empty list."""
        script_list = PlutusV2ScriptList.from_list([])
        assert script_list is not None
        assert len(script_list) == 0

    def test_can_create_from_single_script(self):
        """Test that PlutusV2ScriptList can be created from single script."""
        script = new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR)
        script_list = PlutusV2ScriptList.from_list([script])
        assert len(script_list) == 1

    def test_can_create_from_multiple_scripts(self):
        """Test that PlutusV2ScriptList can be created from multiple scripts."""
        scripts = [
            new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR),
            new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR),
            new_default_plutus_v2_script(PLUTUS_V2_SCRIPT3_CBOR),
        ]
        script_list = PlutusV2ScriptList.from_list(scripts)
...
class TestPlutusV2ScriptListFromCbor:
    ...
    def test_deserialized_scripts_match_expected_cbor(self):
        ...
        expected_cbors = [
            PLUTUS_V2_SCRIPT1_CBOR,
            PLUTUS_V2_SCRIPT2_CBOR,
            PLUTUS_V2_SCRIPT3_CBOR,
            PLUTUS_V2_SCRIPT4_CBOR,
        ]
...
class TestPlutusV2ScriptListToCbor:
    ...
    def test_can_serialize_list_with_four_scripts(self):
        ...
        script_list = PlutusV2ScriptList()
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR))
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT3_CBOR))
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT4_CBOR))
...
class TestPlutusV2ScriptListAdd:
    """Tests for PlutusV2ScriptList.add() method."""
...
class TestPlutusV2ScriptListAppend:
    """Tests for PlutusV2ScriptList.append() method."""
...
class TestPlutusV2ScriptListGet:
    """Tests for PlutusV2ScriptList.get() method."""
...
class TestPlutusV2ScriptListLen:
    """Tests for PlutusV2ScriptList.__len__() method."""
...
class TestPlutusV2ScriptListIter:
    """Tests for PlutusV2ScriptList.__iter__() method."""
...
class TestPlutusV2ScriptListGetItem:
    """Tests for PlutusV2ScriptList.__getitem__() method."""
...
class TestPlutusV2ScriptListBool:
    """Tests for PlutusV2ScriptList.__bool__() method."""
    ...
    def test_can_use_in_if_statement(self):
        ...
        empty_list = PlutusV2ScriptList()
        if empty_list:
            pytest.fail("Empty list should be falsy")

        non_empty_list = PlutusV2ScriptList()
        non_empty_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        if not non_empty_list:
            pytest.fail("Non-empty list should be truthy")


class TestPlutusV2ScriptListIndex:
    """Tests for PlutusV2ScriptList.index() method."""
    ...
    def test_raises_value_error_if_not_found(self):
        ...
        script_list = PlutusV2ScriptList()
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        script_to_find = new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR)

        with pytest.raises(ValueError):
            script_list.index(script_to_find)


class TestPlutusV2ScriptListCount:
    """Tests for PlutusV2ScriptList.count() method."""
    ...
    def test_count_returns_zero_if_not_found(self):
        ...
        script_list = PlutusV2ScriptList()
        script_list.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        script_to_count = new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR)
...
class TestPlutusV2ScriptListReversed:
    """Tests for PlutusV2ScriptList.__reversed__() method."""
...
class TestPlutusV2ScriptListRepr:
    """Tests for PlutusV2ScriptList.__repr__() method."""

    def test_repr_contains_class_name(self):
        """Test that __repr__ contains the class name."""
        script_list = PlutusV2ScriptList()
        repr_str = repr(script_list)
        assert "PlutusV2ScriptList" in repr_str
...
class TestPlutusV2ScriptListContextManager:
    """Tests for PlutusV2ScriptList context manager protocol."""

    def test_can_use_as_context_manager(self):
        """Test that PlutusV2ScriptList can be used as a context manager."""
        with PlutusV2ScriptList() as script_list:
            assert script_list is not None
            assert len(script_list) == 0
...
class TestPlutusV2ScriptListEdgeCases:
    ...
    def test_multiple_lists_are_independent(self):
        ...
        list1.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        assert len(list1) == 1
        assert len(list2) == 0
    ...
    def test_can_create_list_from_existing_list(self):
        ...
        list1 = PlutusV2ScriptList()
        list1.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        list1.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR))

        list2 = PlutusV2ScriptList.from_list(list(list1))
        assert len(list2) == 2

    def test_cbor_roundtrip_preserves_all_scripts(self):
        """Test that CBOR roundtrip preserves all scripts."""
        original = PlutusV2ScriptList()
        original.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT1_CBOR))
        original.add(new_default_plutus_v2_script(PLUTUS_V2_SCRIPT2_CBOR))

        cbor_hex = original.to_cbor()
        deserialized = PlutusV2ScriptList.from_cbor(cbor_hex)
...
class TestPlutusV2ScriptListJsonSerialization:
    """Tests for PlutusV2ScriptList CIP-116 JSON serialization."""
    ...
    def test_single_script_serializes_correctly(self):
        ...
        assert '"language":"plutus_v2"' in json_str
    ...
    def test_multiple_scripts_serialize_correctly(self):
        ...
        script_list = PlutusV2ScriptList()
        script1 = PlutusV2Script.new(bytes([0x00, 0x01]))
        script2 = PlutusV2Script.new(bytes([0xAA, 0xBB, 0xCC]))
...
class TestPlutusV2ScriptListSequenceCompliance:
