"""
Vote for Proposal as DRep (Script Hash) Example

This example demonstrates how to vote for a governance proposal as a
DRep identified by a script hash.
...
def main() -> None:
    ...
    print("=" * 60)
    print("Vote for Proposal as a DRep (Script Hash)")
    print("=" * 60)
    print("This example votes for a proposal as a Script Hash DRep.")
    print()
    ...
    voter = Voter.new(VoterType.DREP_SCRIPT_HASH, script_credential)
    voting_procedure = VotingProcedure.new(Vote.YES, ANCHOR)

    register_drep(wallet, provider, script, redeemer, drep)
    vote_as_drep(wallet, provider, gov_action_id, voter, voting_procedure, script, redeemer)
