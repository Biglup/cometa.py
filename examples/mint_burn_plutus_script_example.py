...
This example demonstrates how to mint two CIP-025 tokens using Plutus V3 scripts
...
def mint(wallet, provider, script, redeemer, asset_id1: bytes, asset_id2: bytes) -> None:
    """
    Mints two tokens using Cometa's transaction builder and sends them
    to the wallet's address.
    """
    ...
    transaction = builder \
        ...
        .expires_in(3600) \
        .mint_token(policy_id=policy1, asset_name=name1, amount=1, redeemer=redeemer) \
        .mint_token(policy_id=policy2, asset_name=name2, amount=1, redeemer=redeemer) \
        .add_script(script) \
...
def burn(wallet, provider, script, redeemer, asset_id1: bytes, asset_id2: bytes) -> None:
    ...
    transaction = builder \
        .expires_in(3600) \
        .mint_token_with_id(asset_id=asset_id1, amount=-1, redeemer=redeemer) \
        .mint_token_with_id(asset_id=asset_id2, amount=-1, redeemer=redeemer) \
        .add_script(script) \
        .build()
...
def main() -> None:
    """Example of minting and burning tokens with a Plutus script using Cometa."""
    print("=" * 60)
    print("Mint & Burn with Plutus Scripts Example")
    print("=" * 60)
    print("This example mints two CIP-025 tokens and burns them afterwards using Plutus scripts.")
    print()
    ...
    mint(wallet, provider, script, redeemer, asset_id1, asset_id2)
    burn(wallet, provider, script, redeemer, asset_id1, asset_id2)
